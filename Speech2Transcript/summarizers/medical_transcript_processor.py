...
class MedicalTranscriptProcessor(BaseExtractor):
    ...
    def __init__(
        ...
        # Initialize CCM template extractor
        self.ccm_extractor = CCMTemplateExtractor(
            ner_model=ner_model,
            device=device,
    ...
    def process_transcript(
        ...
        # Step 9: Extract CCM-specific information using advanced NLP
        ccm_data = self.ccm_extractor.extract(all_text, entities, segments)
        ...
        # Generate CCM note using advanced NLP approach
        ccm_note = generate_ccm_note(results, ccm_template_path, self.logger)
        results["ccm_note"] = ccm_note
        ...
        if output_path:
            try:
                ...
                # Save CCM note
                ccm_path = output_path.replace('.json', '_ccm_note.txt')
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        # Add CCM conditions if available
        ccm_data = results.get("ccm_data", {})
        if not health_status.get("conditions") and ccm_data.get("chronic_conditions"):
            condition_texts = [c["name"] for c in ccm_data["chronic_conditions"]]
            if condition_texts:
                summary_parts.append(f"Patient has the following chronic conditions: {', '.join(condition_texts)}.")
